...
=head 1 Jobsdoer::Doers::FetchSnmp::Munge
...
package Jobsdoer::Doers::FetchSnmp::Munge;
...
sub new {
    ...
    my %selfhash; 
    $selfhash{'data'} = $data;
    $selfhash{'cacheObj'} = Cache::Memcached->new(
        { 'servers' => ['chrisp01.dev:11211'] }
    );
...
sub perSecond {
    ...
    if ($prevValHash) {
	    ...
	    if ( $counterbits and $bigAdjVal < $bigPrevVal ) {
		    ...
		    print localtime." - Detected rolled counter for $metricName: Doing $bigAdjVal += $bigCounterMax = ";
	    ...
	    if ( $bigAdjVal and $bigAdjVal->is_pos() and $timeDiff ) {
			...
			print "WARNING: processing $metricName, $adjustedVal - $prevValHash->{'value'} = $bigAdjVal\n";
		}
		else {
			$mungedResult = '0';
		}
    ...
    $self->_storeCache($metricName, {'value' => $value, 'timestamp' => $timestamp});
